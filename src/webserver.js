...
server.on('error', (err) => {
	if (err.code === 'EADDRINUSE') {
		winston.error(`magicforum address in use, exiting...\n${err.stack}`);
	} else {
...
async function listen() {
	...
	if (Array.isArray(port)) {
		...
		winston.warn('[startup] If you want to start magicforum on multiple ports please use loader.js');
	...
	if (isSocket) {
		...
		try {
			...
			winston.error(`[startup] magicforum was unable to secure domain socket access (${socketPath})\n${err.stack}`);
			throw err;
		}
	}
	...
	return new Promise((resolve, reject) => {
		server.listen(...args.concat([function (err) {
			...
			if (err) {
				winston.error(`[startup] magicforum was unable to listen on: ${chalk.yellow(onText)}`);
				reject(err);
			}

			winston.info(`📡 magicforum is now listening on: ${chalk.yellow(onText)}`);
