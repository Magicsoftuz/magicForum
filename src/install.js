...
questions.main = [
	{
		name: 'url',
		description: 'URL used to access this magicforum',
		default:
	...
	{
		name: 'secret',
		description: 'Please enter a magicforum secret',
...
function checkSetupFlagEnv() {
	...
	const envConfMap = {
		magicforum_URL: 'url',
		magicforum_PORT: 'port',
		magicforum_ADMIN_USERNAME: 'admin:username',
		magicforum_ADMIN_PASSWORD: 'admin:password',
		magicforum_ADMIN_EMAIL: 'admin:email',
		magicforum_DB: 'database',
		magicforum_DB_HOST: 'host',
		magicforum_DB_PORT: 'port',
		magicforum_DB_USER: 'username',
		magicforum_DB_PASSWORD: 'password',
		magicforum_DB_NAME: 'database',
		magicforum_DB_SSL: 'ssl',
	};
	...
	if (Object.keys(envConfMap).some(key => envKeys.includes(key))) {
		...
		Object.entries(process.env).forEach(([evName, evValue]) => { // get setup values from env
			if (evName.startsWith('magicforum_DB_')) {
				setupVal[`${process.env.magicforum_DB}:${envConfMap[evName]}`] = evValue;
			} else if (evName.startsWith('magicforum_')) {
...
async function enableDefaultTheme() {
	...
	const defaultTheme = nconf.get('defaultTheme') || 'magicforum-theme-persona';
...
async function createWelcomePost() {
	...
	if (!parseInt(numTopics, 10)) {
		...
		await Topics.post({
			uid: 1,
			cid: 2,
			title: 'Welcome to your magicforum!',
			content: content,
		});
	}
}
...
async function enableDefaultPlugins() {
	...
	let defaultEnabled = [
		'magicforum-plugin-composer-default',
		'magicforum-plugin-markdown',
		'magicforum-plugin-mentions',
		'magicforum-widget-essentials',
		'magicforum-rewards-essentials',
		'magicforum-plugin-emoji',
		'magicforum-plugin-emoji-android',
	];
...
install.setup = async function () {
	try {
		...
		if (err) {
			winston.warn(`magicforum Setup Aborted.\n ${err.stack}`);
			process.exit(1);
		}
	}
};
